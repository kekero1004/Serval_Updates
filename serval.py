...
class Serval(object):
    ...
    def __init__(self, iface):
        ...
        # Non-modal dialogs for interpolation and terrain filtering
        self.interpolate_edges_dialog = None
        self.interpolate_vertices_dialog = None
        self.terrain_filter_dialog = None
        self.debug = DEBUG
    ...
    def initGui(self):
        ...
        self.fill_voids_btn = self.add_action(
            'void-svgrepo-com.svg',
            text="Fill Voids (NoData Areas)",
            callback=self.apply_fill_voids,
            add_to_toolbar=self.toolbar,
            checkable=False, )

        self.terrain_filter_btn = self.add_action(
            'map-solid-svgrepo-com.svg',
            text="Terrain Filter (DSM to DTM)",
            callback=self.apply_terrain_filter,
            add_to_toolbar=self.toolbar,
            checkable=False, )

        self.outlier_filter_btn = self.add_action(
            'outlierFilter.png',
            text="Outlier Filter",
            callback=self.apply_outlier_filter,
            add_to_toolbar=self.toolbar,
            checkable=False, )

        self.interpolate_edges_btn = self.add_action(
            'interpolates.png',
            text="Interpolate from Edges",
            callback=self.apply_interpolate_edges,
            add_to_toolbar=self.toolbar,
            checkable=False, )

        self.interpolate_vertices_btn = self.add_action(
            'interpolatesVertices.png',
            text="Interpolate from Vertices",
            callback=self.apply_interpolate_vertices,
            add_to_toolbar=self.toolbar,
            checkable=False, )

        self.average_filter_btn = self.add_action(
            'average.svg',
            text="Average Filter",
            callback=self.apply_average_filter,
            add_to_toolbar=self.toolbar,
            checkable=False, )

        self.constrained_filter_btn = self.add_action(
            'apply_low_pass_filter.svg',
            text="Constrained Filter",
            callback=self.apply_constrained_filter,
            add_to_toolbar=self.toolbar,
            checkable=False, )

        self.median_filter_btn = self.add_action(
            'chart-median.svg',
            text="Median Filter",
            callback=self.apply_median_filter,
            add_to_toolbar=self.toolbar,
            checkable=False, )
    ...
    def set_active_raster(self):
        ...
        # Clean up any open non-modal dialogs when raster changes
    ...
    def apply_fill_voids(self):
        """Apply Fill Voids filter to selection."""
        if not self.selection_tool.selected_geometries:
            self.uc.bar_warn("No selection for raster layer. Select some cells and retry...")
            return
        
        # Get parameters from user
        method, ok = QInputDialog.getItem(None, "Fill Voids Parameters", 
                                         "Interpolation Method:", 
                                         ["idw", "bilinear", "kriging", "natural_neighbor"], 
                                         0, False)
        if not ok:
            return
            
        search_radius, ok = QInputDialog.getInt(None, "Fill Voids Parameters", 
                                               "Search Radius (pixels):", 10, 1, 100)
        if not ok:
            return
            
        self._apply_advanced_filter('fill_voids', method=method, search_radius=search_radius)

    def apply_terrain_filter(self):
        """Apply Terrain Filter to selection with non-modal PMF dialog."""
        if not self.selection_tool.selected_geometries:
            self.uc.bar_warn("No selection for raster layer. Select some cells and retry...")
            return
            
        # Import the dialog
        from .terrain_filter_dialog import TerrainFilterDialog
        
        # Close existing dialog if open
        if hasattr(self, 'terrain_filter_dialog') and self.terrain_filter_dialog:
            self.terrain_filter_dialog.close()
            
        # Create and show the non-modal dialog
        self.terrain_filter_dialog = TerrainFilterDialog(self.iface.mainWindow())
        
        # Connect signals for non-modal operation
        self.terrain_filter_dialog.filtering_requested.connect(self._on_terrain_filtering_requested)
        self.terrain_filter_dialog.dialog_closed.connect(self._on_terrain_dialog_closed)
        
        # Show the dialog (non-modal)
        self.terrain_filter_dialog.show()
        self.terrain_filter_dialog.raise_()
        self.terrain_filter_dialog.activateWindow()

    def apply_outlier_filter(self):
        """Apply Outlier Filter to selection."""
        if not self.selection_tool.selected_geometries:
            self.uc.bar_warn("No selection for raster layer. Select some cells and retry...")
            return
            
        # Get parameters from user
        method, ok = QInputDialog.getItem(None, "Outlier Filter Parameters", 
                                         "Detection Method:", 
                                         ["statistical", "mad", "iqr"], 
                                         0, False)
        if not ok:
        ...
        threshold, ok = QInputDialog.getDouble(None, "Outlier Filter Parameters", 
                                              "Threshold:", 2.5, 1.5, 4.0, 1)
        if not ok:
            return
            
        replacement, ok = QInputDialog.getItem(None, "Outlier Filter Parameters", 
                                              "Replacement Method:", 
                                              ["median", "mean", "interpolate", "nodata"], 
                                              0, False)
        if not ok:
            return
            
        self._apply_advanced_filter('outlier_filter', method=method, threshold=threshold, replacement=replacement)

    def apply_interpolate_edges(self):
        """Apply Interpolate from Edges to selection with non-modal dialog."""
        if not self.selection_tool.selected_geometries:
            self.uc.bar_warn("No selection for raster layer. Select some cells and retry...")
            return
            
        # Import the dialog
        from .interpolation_dialogs import InterpolateFromEdgesDialog
        
        # Close existing dialog if open
        if hasattr(self, 'interpolate_edges_dialog') and self.interpolate_edges_dialog:
            self.interpolate_edges_dialog.close()
            
        # Create and show the non-modal dialog
        self.interpolate_edges_dialog = InterpolateFromEdgesDialog(self.iface.mainWindow())
        
        # Connect signals for non-modal operation
        self.interpolate_edges_dialog.interpolation_requested.connect(self._on_edges_interpolation_requested)
        self.interpolate_edges_dialog.dialog_closed.connect(self._on_edges_dialog_closed)
        
        # Show the dialog (non-modal)
        self.interpolate_edges_dialog.show()
        self.interpolate_edges_dialog.raise_()
        self.interpolate_edges_dialog.activateWindow()

    def apply_interpolate_vertices(self):
        """Apply Interpolate from Vertices to selection with non-modal dialog."""
        if not self.selection_tool.selected_geometries:
            self.uc.bar_warn("No selection for raster layer. Select some cells and retry...")
            return
            
        # Import the dialog
        from .interpolation_dialogs import InterpolateFromVerticesDialog
        
        # Close existing dialog if open
        if hasattr(self, 'interpolate_vertices_dialog') and self.interpolate_vertices_dialog:
            self.interpolate_vertices_dialog.close()
            
        # Create and show the non-modal dialog
        self.interpolate_vertices_dialog = InterpolateFromVerticesDialog(self.iface.mainWindow())
        
        # Connect signals for non-modal operation
        self.interpolate_vertices_dialog.interpolation_requested.connect(self._on_vertices_interpolation_requested)
        self.interpolate_vertices_dialog.dialog_closed.connect(self._on_vertices_dialog_closed)
        
        # Show the dialog (non-modal)
        self.interpolate_vertices_dialog.show()
        self.interpolate_vertices_dialog.raise_()
        self.interpolate_vertices_dialog.activateWindow()

    def _on_edges_interpolation_requested(self, parameters):
        """Handle interpolation request from non-modal edges dialog."""
        if not self.selection_tool.selected_geometries:
            self.uc.bar_warn("No selection available. Please select some cells first.")
            return
            
        try:
            self._apply_advanced_filter('interpolate_edges_advanced', **parameters)
            self.uc.bar_info("Edge interpolation applied successfully")
        except Exception as e:
            self.uc.show_warn(f"Edge interpolation failed: {str(e)}")
            
    def _on_edges_dialog_closed(self):
        """Handle edges dialog close event."""
        if hasattr(self, 'interpolate_edges_dialog'):
            self.interpolate_edges_dialog = None
            
    def _on_vertices_interpolation_requested(self, parameters):
        """Handle interpolation request from non-modal vertices dialog."""
        if not self.selection_tool.selected_geometries:
            self.uc.bar_warn("No selection available. Please select some cells first.")
            return
            
        try:
            self._apply_advanced_filter('interpolate_vertices_advanced', **parameters)
            self.uc.bar_info("Vertex interpolation applied successfully")
        except Exception as e:
            self.uc.show_warn(f"Vertex interpolation failed: {str(e)}")
            
    def _on_vertices_dialog_closed(self):
        """Handle vertices dialog close event."""
        if hasattr(self, 'interpolate_vertices_dialog'):
            self.interpolate_vertices_dialog = None
            
    def _on_terrain_filtering_requested(self, parameters):
        """Handle terrain filtering request from non-modal dialog."""
        if not self.selection_tool.selected_geometries:
            self.uc.bar_warn("No selection available. Please select some cells first.")
            return
            
        try:
            self._apply_advanced_filter('terrain_filter_pmf', **parameters)
            self.uc.bar_info("Terrain filter applied successfully")
        except Exception as e:
            self.uc.show_warn(f"Terrain filtering failed: {str(e)}")
            
    def _on_terrain_dialog_closed(self):
        """Handle terrain dialog close event."""
        if hasattr(self, 'terrain_filter_dialog'):
            self.terrain_filter_dialog = None

    def apply_average_filter(self):
        """Apply Average Filter to selection."""
        if not self.selection_tool.selected_geometries:
            self.uc.bar_warn("No selection for raster layer. Select some cells and retry...")
            return
            
        # Get parameters from user
        kernel_size, ok = QInputDialog.getItem(None, "Average Filter Parameters", 
                                              "Kernel Size:", 
        ...
        kernel_size = int(kernel_size.split('x')[0])
        
        weights, ok = QInputDialog.getItem(None, "Average Filter Parameters", 
                                          "Weight Type:", 
                                          ["uniform", "gaussian", "distance"], 
                                          0, False)
        if not ok:
            return
            
        self._apply_advanced_filter('average_filter', kernel_size=kernel_size, weights=weights)

    def apply_constrained_filter(self):
        """Apply Constrained Filter to selection."""
        if not self.selection_tool.selected_geometries:
            self.uc.bar_warn("No selection for raster layer. Select some cells and retry...")
            return
            
        # Get parameters from user
        max_change, ok = QInputDialog.getDouble(None, "Constrained Filter Parameters", 
                                               "Maximum Change (meters):", 1.0, 0.1, 10.0, 1)
        if not ok:
            return
            
        self._apply_advanced_filter('constrained_filter', max_change=max_change)

    def apply_median_filter(self):
        """Apply Median Filter to selection."""
        if not self.selection_tool.selected_geometries:
            self.uc.bar_warn("No selection for raster layer. Select some cells and retry...")
            return
            
        # Get parameters from user
        kernel_size, ok = QInputDialog.getItem(None, "Median Filter Parameters", 
                                              "Kernel Size:", 
                                              ["3x3", "5x5", "7x7", "9x9"], 
                                              0, False)
        if not ok:
        ...
        shape, ok = QInputDialog.getItem(None, "Median Filter Parameters", 
                                        "Kernel Shape:", 
        ...
        self._apply_advanced_filter('median_filter', kernel_size=kernel_size, shape=shape)

    def _apply_advanced_filter(self, filter_type, **kwargs):
        """Apply advanced filter to selected area."""
        QApplication.setOverrideCursor(Qt.WaitCursor)
        try:
            self.handler.select(self.selection_tool.selected_geometries, all_touched_cells=self.all_touched)
            self.handler.write_block_advanced_filter(filter_type, **kwargs)
            self.raster.triggerRepaint()
        except Exception as e:
            self.uc.show_warn(f"Filter application failed: {str(e)}")
        finally:
            QApplication.restoreOverrideCursor()
            
    def cleanup_dialogs(self):
        """Clean up non-modal dialogs when plugin is unloaded or raster changes."""
        if hasattr(self, 'interpolate_edges_dialog') and self.interpolate_edges_dialog:
            self.interpolate_edges_dialog.close()
            self.interpolate_edges_dialog = None
